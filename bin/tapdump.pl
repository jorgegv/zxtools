...
sub output_data_block_info {
    ...
    if ( $block_type eq 'HEADER' ) {
        ...
        if ( $data_type eq 'PROGRAM' ) {
            ...
            printf "  [Header] Type: %s, File Name: \"%s\", Length: %d, Autostart: %d, Program Length: %d\n",
                $data_type, $filename, $length, $auto_start, $program_length;
        }
        if ( $data_type eq 'NUMVAR' ) {
            my $var_name = @$data[ 13 .. 14 ];
            printf "  [Header] Type: %s, File Name: \"%s\", Length: %d, Variable Name: %s\n",
        ...
        if ( $data_type eq 'CODE' ) {
            ...
            printf "  [Header] Type: %s, File Name: \"%s\", Length: %d, Start Address: %d (0x%04X)\n",
...
while ( $pos < $file_size ) {
    ...
    $pos += $size - 2;
    my $cksum = $bytes->[ $pos++ ];
    printf( "Block #%d: pos = %d, size = %d, flag = %d (%s), cksum = %d\n",
        $num_block++,
        $savepos, $size, $flag, ( $flag ? 'data' : 'header' ), $cksum
    );
